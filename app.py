...
GLOBALTRACKER_RESET_STAMP = os.environ.get("GLOBALTRACKER_RESET_STAMP", "20260502_focused_bot_reset_02")
...
def login():
    ...
    auth_redirect = f"{authorize_url}?{query}"
    print("LOGIN REDIRECT:", auth_redirect)
...
def callback():
    ...
    try:
        ...
        print("CALLBACK DEBUG: token status =", token_res.status_code)
        print("CALLBACK DEBUG: token body =", token_res.text[:500])
        token_res.raise_for_status()
        tokens = token_res.json()
        userinfo_url = f"https://{AUTH0_DOMAIN}/userinfo"
        print("CALLBACK DEBUG: userinfo fetch start")
        userinfo_res = requests.get(
            userinfo_url,
            headers={"Authorization": f"Bearer {tokens.get('access_token','')}"}, timeout=10
        )
        print("CALLBACK DEBUG: userinfo status =", userinfo_res.status_code)
        print("CALLBACK DEBUG: userinfo body =", userinfo_res.text[:500])
        userinfo_res.raise_for_status()
        session["user"] = userinfo_res.json()
        sync_authenticated_local_remote_account(session["user"])
        email = (session["user"].get("email") or "").strip()
        name = (session["user"].get("name") or session["user"].get("nickname") or "").strip()
        final_redirect = f"https://supportrd.com/local-remote?logged_in=true&login=confirmed&email={requests.utils.quote(email)}&name={requests.utils.quote(name)}"
        print("CALLBACK SUCCESS REDIRECT:", final_redirect)
...
def logout():
    ...
    if not AUTH0_DOMAIN or not AUTH0_CLIENT_ID:
        return redirect("https://supportrd.com/local-remote")
    return redirect(
