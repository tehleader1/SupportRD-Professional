(function(){
  ...
  function patchRoutes(){if(root.__settingsRoutePatched)return;root.__settingsRoutePatched=true;const old=root.renderFunctionalPanel;root.renderFunctionalPanel=function(route){if(route==='settings'){const stage=document.querySelector('#remoteStage');document.querySelectorAll('[data-route]').forEach(b=>b.classList.toggle('active',b.dataset.route==='settings'));if(stage)stage.innerHTML=renderSettings();return}return old?.(route)};window.renderPanel=root.renderFunctionalPanel}
  function patchButtons(){document.querySelectorAll('[data-route="settings"]').forEach(b=>{b.textContent='Settings'});}
